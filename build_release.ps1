# PowerShell script to build all BaoTools v105.5 release packages
$ErrorActionPreference = "Stop"

Write-Host "===============================================" -ForegroundColor Cyan
Write-Host " Building BaoTools v105.5 Release Packages" -ForegroundColor Cyan
Write-Host "===============================================" -ForegroundColor Cyan
...
# 5. Assemble to release_v105.5
Write-Host "`nAssembling release assets to release_v105.5/ ..." -ForegroundColor Green
New-Item -ItemType Directory -Force -Path "release_v105.5" | Out-Null
Copy-Item "out_single_standalone\BaoTools.exe" "release_v105.5\BaoTools.exe" -Force
if (Test-Path "out_setup\BaoTools_Setup.exe") {
    Copy-Item "out_setup\BaoTools_Setup.exe" "release_v105.5\BaoTools_Setup.exe" -Force
}
Compress-Archive -Path "out_portable\*" -DestinationPath "release_v105.5\BaoTools_v105.5_Portable.zip" -Force

Write-Host "`nAll release assets successfully created in release_v105.5/:" -ForegroundColor Green
Get-ChildItem "release_v105.5" | Select-Object Name, Length, LastWriteTime | Format-Table -AutoSize
