...
```powershell
dotnet build BaoToolsGui.sln
dotnet run --project src/BaoToolsGui/BaoToolsGui.csproj
```

For iterative development:

```powershell
dotnet watch --project src/BaoToolsGui/BaoToolsGui.csproj
```
...
| Path | Contents |
|---|---|
| `src/BaoToolsGui/` | The application: `Views/` (XAML), `ViewModels/`, `Services/`, `Models/`, `Resources/` (localization) |
| `src/BaoToolsGui/AppConfig.cs` | All compiled-in endpoints, mirrors and public client values |
| `tests/BaoToolsGui.Tests/` | xUnit tests |
...
1. Add the key to the English source, `src/BaoToolsGui/Resources/Strings.resx`.
...
[`src/BaoToolsGui/Resources/README.md`](src/BaoToolsGui/Resources/README.md) for the translator
...
`Millennium.callServerMethod("baotools", "<Name>", args)` and needs a matching entry in
