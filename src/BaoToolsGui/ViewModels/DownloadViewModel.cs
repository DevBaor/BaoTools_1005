...
public partial class DownloadViewModel : ObservableObject
{
    ...
    // Set while a baotools://install/silent/<id> install runs so the download path skips the interactive
    // overwrite-confirm overlay (there's no surfaced window for the user to confirm it on).
    private bool _silentInstall;

    /// <summary>
    /// Called by the protocol handler for baotools://install/&lt;appid&gt;. Seeds the app, fetches details,
    ...
    public DownloadViewModel(BaoToolsApiClient api, HubcapService hubcap, SettingsService settings,
    ...
    private async Task FetchDetailsDebouncedAsync(string appid)
    {
        ...
        try
        {
            ...
            OnPropertyChanged(nameof(GenresText));
            if (Details is null) { Error = "Không tìm thấy thông tin game này."; return; }
            if (HasDetails) await FetchCommand.ExecuteAsync(null);
        }
        catch (OperationCanceledException) { }
        catch { Details = null; Error = "Lỗi kết nối khi tải dữ liệu từ Steam."; }
    }
    ...
    private static LuaContents? ExtractLuaFromZip(string zipPath, long appId)
    {
        try
        {
            ...
            string tmp = Path.Combine(Path.GetTempPath(), $"baotools_diff_{appId}_{Guid.NewGuid():N}.lua");
