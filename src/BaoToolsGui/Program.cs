...
public static class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        // Register the baotools:// protocol handler so browser links can open the app.
        Services.ProtocolService.Register();

        // Check if we were launched with a baotools:// protocol URL.
        ...
        if (args is { Length: > 0 })
        {
            ...
            {
                if (arg.StartsWith("baotools://", StringComparison.OrdinalIgnoreCase))
                {
                    protocolUrl = arg;
                }
        ...
        // Only one instance may run: two copies share %AppData%\BaoToolsGui (auth.dat,
        // caches) and would lock each other's files (e.g. a token write failing on login).
        using var single = new Mutex(initiallyOwned: true, "BaoToolsGui.SingleInstance", out bool isFirst);
        if (!isFirst)
        {
    ...
    private const string ShowWindowEventName = "BaoToolsGui.ShowWindow";
    ...
    private const string EnableTrayLockEventName = "BaoToolsGui.EnableTrayLock";
    ...
    private const string RecheckUpdatesEventName = "BaoToolsGui.RecheckUpdates";
    ...
    /// <summary>Read just the "Language" key from %AppData%\BaoToolsGui\settings.json without spinning up
