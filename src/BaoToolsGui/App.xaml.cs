...
public partial class App : Application
{
    ...
    private async Task CheckLaunchOptionDriftAsync()
    {
        try
        {
            ...
            Dispatcher.Invoke(() => toast.ShowAction(
                BaoToolsGui.Resources.Strings.Launch_Drift_Title,
                string.Format(BaoToolsGui.Resources.Strings.Launch_Drift_Body, drifted.Count),
                BaoToolsGui.Resources.Strings.Launch_Drift_Action,
        ...
        Services.AppInfo.LaunchOptionsService launch, IReadOnlyList<int> drifted, ToastService toast)
    {
        ...
        if (result.Ok)
            toast.Show(BaoToolsGui.Resources.Strings.Launch_Title,
                result.SteamWasRunning
                    ? BaoToolsGui.Resources.Strings.Launch_Applied_Restarted
                    : BaoToolsGui.Resources.Strings.Launch_Applied);
        else
            toast.Show(BaoToolsGui.Resources.Strings.Launch_Title,
                string.Format(BaoToolsGui.Resources.Strings.Launch_ApplyFailed, result.Error), error: true);
    }
            ...
            try
            {
                ...
                if (st.UpdateAvailable)
                {
                    if (!st.DllMatches)
                    {
                        ...
                        Dispatcher.Invoke(() => t.Show("BaoTools", "Updating plugin. Steam will restart."));
                    }
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        _ = System.Threading.Tasks.Task.Run(() =>
        {
            try
            {
                string legacy = System.IO.Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads", "BaoTools");
        ...
        Func<Task> navigateToSignIn = () =>
        {
            Dispatcher.Invoke(() =>
            {
                settingsVm.LoginRequiredMessage = BaoToolsGui.Resources.Strings.Settings_LoginRequired;
        ...
        settingsVm.RequestRestartPrompt = () => Dispatcher.Invoke(() =>
            toast.ShowAction(
                BaoToolsGui.Resources.Strings.Lang_Changed_Title,
                BaoToolsGui.Resources.Strings.Lang_Changed_Body,
                BaoToolsGui.Resources.Strings.Lang_Changed_Restart,
        ...
        // baotools://install/<id> are literally the same path and can't drift apart later.
        // DropInstallViewModel is transient, so Home and Add each hold their own instance.
        Func<long, Task> installByAppId = appId =>
        {
            Dispatcher.Invoke(() => HandleProtocolUrl($"baotools://install/{appId}"));
            return Task.CompletedTask;
        };
        ...
        // A silent install launch (baotools://install/silent/<id>) runs headless: stay in the tray and
    ...
    /// <summary>Route a baotools:// protocol URL to the appropriate page and action.</summary>
