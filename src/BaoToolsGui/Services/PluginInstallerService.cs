...
/// <summary>
/// Installs / updates / removes the BaoTools store-page plugin from GitHub releases (the app is the plugin
/// MANAGER: it doesn't bundle the frontend). Each release of <c>madoiscool/LTSP</c> carries
/// <c>plugin.zip</c> (the frontend, extracted to %AppData%\BaoToolsGui\plugin, where CefInjectorService
...
/// "launch BaoTools.exe when Steam opens", with no CDP hook, no load-timing race, and no dual-slot
...
public class PluginInstallerService(SteamService steam, GithubProxy gh, CefInjectorService injector)
{
    ...
    // payload an extra time (double BaoTools launch), and bcrypt/dbghelp specifically would still install
    ...
    private static string FrontendDir => Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "BaoToolsGui", "plugin");
    private static string LuatoolsJsPath => Path.Combine(FrontendDir, "public", "baotools.js");
    ...
    // Drop `.baotools-dll-update-disabled` in the Steam root to stop the app from replacing any installed
    ...
    // restart. Mirrors the DLL's own `.baotools-cdp-hook-disabled` marker idiom.
    private const string DllUpdateDisabledMarker = ".baotools-dll-update-disabled";
    ...
    private sealed class Manifest
    {
        ...
        /// so uninstall can restore Millennium's baotools plugin verbatim.</summary>
    ...
    public async Task<(bool ok, string? error)> InstallAsync(IProgress<double?>? progress, CancellationToken ct = default)
    {
        ...
        string tmp = Path.Combine(Path.GetTempPath(), "baotools-plugin-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            // 1) Frontend → %AppData%\BaoToolsGui\plugin (fresh).
            ...
            // below. A relaunched BaoTools.exe would hit the single-instance mutex against this very
            ...
            // Testing switch: when `.baotools-dll-update-disabled` is present, never touch any on-disk DLL
            ...
            if (dllNeedsUpdate)
            {
                ...
                // payload an extra time (double BaoTools launch / CDP hook).
                foreach (var legacy in LegacyDllPaths)
                    if (File.Exists(legacy)) { try { File.Delete(legacy); } catch { /* locked/again next time */ } }

                // 3) A live Millennium baotools plugin would inject the frontend redundantly. Disable it in
    ...
    public Task<(bool ok, string? error)> UninstallAsync(CancellationToken ct = default)
    {
        return Task.Run(async () =>
        {
            try
            {
                ...
                // Give Millennium its baotools plugin back: we're the ones who disabled it. Steam is
    ...
    // ── Millennium coexistence: disable its baotools plugin via config (reversible), not folder-rename ──

    private const string MillenniumPluginName = "baotools";
    private const string MillenniumDisabledName = MillenniumPluginName + ".disabled-by-baotools";
    ...
    /// token is the plugin id; Millennium versions store it as <c>baotools</c>, <c>baotools LUA</c>
    /// (name + backend), or (from the old rename) <c>baotools.disabled-by-baotools[ LUA]</c>.</summary>
    ...
    /// <summary>Undo the previous folder-rename approach: bring <c>plugins\baotools</c> back to a normal
    ...
    /// up so <c>public/baotools.js</c> sits directly under FrontendDir.</summary>
    private static void NormalizeFrontendLayout()
    {
        ...
        {
            string jsContent = File.ReadAllText(luaPath);
