...
{
    private const string ProtocolName = "baotools";

    private static readonly string PendingFile = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
        "BaoToolsGui", "protocol_url.tmp");
    ...
    public static void Register()
    {
        try
        {
            ...
            protoKey.SetValue("", "URL:BaoTools Protocol");
    ...
    public static (string? Action, long? AppId, bool Silent) Parse(string url)
    {
        ...
        // baotools://install/silent/<appid> → run the install headless (tray only + a balloon when done).
        bool silent = false;
