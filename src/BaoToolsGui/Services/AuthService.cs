...
public class AuthService
{
    ...
    private static string ResultPage(bool ok, string? error) => $$"""
        <!doctype html>
        <html><head><meta charset="utf-8"><title>BaoTools</title>
        <style>
        ...
        <body><div class="card">
          ...
          <p>{{(ok ? "You can close this tab and return to BaoTools." : WebUtility.HtmlEncode(error ?? "Please try again from the app."))}}</p>
        </div></body></html>
        """;
}
