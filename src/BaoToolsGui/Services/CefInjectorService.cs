...
public class CefInjectorService : IHostedService
{
    ...
    /// <summary>(Re)reads baotools.js + the polyfill from disk into memory. Called once at startup, and
    ...
    /// installed, so this isn't just a theoretical edge case). _baotoolsJs/_polyfillJs are plain strings.
    ...
    public async Task ReloadPluginFilesAsync()
    {
        ...
        var jsPath = FindBaoToolsJs();
        if (jsPath is not null && File.Exists(jsPath))
        {
            _baotoolsJs = await File.ReadAllTextAsync(jsPath, ct);
            _log.LogInformation("Loaded baotools.js ({Length} bytes)", _baotoolsJs.Length);
        }
        else
        {
            _baotoolsJs = "";
            _log.LogWarning("baotools.js not found");
        }
    ...
    private async Task InjectionLoop(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                // ── Slow cadence (~1s): discover store tabs, ensure baotools.js is injected ──
                if (tick % InjectEveryTicks == 0)
                {
                    // Millennium may also be present and running its own plugin loader on this same page.
                    // That's fine now. The polyfill (BuildInlinePolyfill) only takes over "baotools" calls and
                    ...
                    if (!string.IsNullOrWhiteSpace(tabsJson))
                    {
                        ...
                        // Inject baotools.js into every store-page tab whose JS context isn't already alive.
                        ...
                        // cycle instead (window.__BaoToolsReady, set as the last statement of baotools.js's
                        ...
                        foreach (var tab in tabs)
                        {
                            ...
                            // store home) need baotools.js present on the home page too. Steam boots straight
                            // to the home page, which has no "/app/" in its URL, so the old "/app/"-only filter
                            // meant the icon/popup never appeared there. baotools.js itself gates the per-game
                            // "Add via BaoTools" button to /app/ pages internally, so injecting store-wide is safe.
                                ...
                                && !string.IsNullOrEmpty(tab.Id))
                            {
                                ...
                                var alive = await EvaluateReturnAsync(tab.Id!, tab.WebSocketDebuggerUrl!, "String(window.__BaoToolsReady === true)", ct);
                                if (alive != "true")
    ...
    private async Task<string> CallBackendMethod(string method, string argsRaw)
    {
        ...
        {
            ["HasBaoToolsForApp"] = ("GET", "/has/{appid}"),
            ["CheckApisForApp"] = ("POST", "/check-sources/{appid}"),
            ["StartAddViaBaoToolsFromUrl"] = ("POST", "/download/{appid}"),
            ["GetAddViaBaoToolsStatus"] = ("GET", "/download-status/{appid}"),
            ["CancelAddViaBaoTools"] = ("POST", "/cancel/{appid}"),
            ["DeleteBaoToolsForApp"] = ("POST", "/remove/{appid}"),
            ...
            ["StartBaoToolsAdd"] = ("POST", "/add/{appid}"),
            ["GetBaoToolsAddStatus"] = ("GET", "/add-status/{appid}"),
            ["PickBaoToolsAddSource"] = ("POST", "/add-source/{appid}"),
    ...
    /// <summary>Fire-and-forget evaluate (used for injecting the polyfill + baotools.js). Shares the same
    ...
    private string? FindBaoToolsJs()
    {
        string appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        // The frontend is no longer bundled with the app. PluginInstallerService downloads it from
        // GitHub releases into %AppData%\BaoToolsGui\plugin. If it isn't installed yet, nothing injects.
        string[] candidates =
        {
            Path.Combine(appData, "BaoToolsGui", "plugin", "public", "baotools.js"),
            Path.Combine(appData, "BaoToolsGui", "plugin", "baotools.js"),
        };
    ...
    /// Millennium plugin on the page whenever both injectors were active, not just BaoTools. This only takes
    /// over "baotools" calls and passes every other pluginName through to the real callServerMethod
...
if(real&&typeof real.callServerMethod==='function'){
  ...
  real.callServerMethod=function(p,m,a){return p==='baotools'?ltCall(p,m,a):realCall(p,m,a)};
